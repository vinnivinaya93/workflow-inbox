...
export interface ReleaseInboxItemCommand {
  readonly id: InboxItemId;
  readonly actor: ActorId;
}

export class ReleaseInboxItem {
  constructor(private readonly uow: UnitOfWork, private readonly clock: Clock) {}

  async execute(command: ReleaseInboxItemCommand): Promise<InboxItemView> {
    ...
    return this.uow.transaction(async (ctx) => {
      ...
      item.release(command.actor, now);
